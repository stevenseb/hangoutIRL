...
* Request
  * Method: GET
  * URL: /api/groups/:userId <!-- If you are using userId, it should be in the user router "/api/users/:userId. If you want to keep this endpoint in the groups router you can do something like "/api/groups/currentUser"-->
  * Body: none
...
* Request
  * Method: POST
  * URL: /api/groupImages/:groupId <!-- Since we are adding an image to a specific group, we should be in the groups router. Should be something like "/api/groups/:groupId/images"-->
  * Headers:
    * Content-Type: application/json
  * Body:
...
* Request
  * Method: GET
  * URL: /api/venues/:groupId <!-- If we are looking for a specific group's details we need to be in the group router. "/groups/:groupId/venues" We should not mismatch the router venue with a group id.-->
  * Headers:
    * Content-Type: application/json
  * Body: none
...
* Request
  * Method: POST
  * URL: /api/venues/:groupId <!-- If we are creating a venue for a group, we need to be in the group router. "/groups/:groupId/venues" We should not mismatch the router venue with a group id.-->
  * Headers:
    * Content-Type: application/json
  * Body:
...
* Request
  * Method: GET
  * URL: /api/events/:groupId <!-- If we are looking for a specific group's details we need to be in the group router. "/groups/:groupId/events" We should not mismatch the router event with a group id.-->
  * Body: none
...
* Request
  * Method: POST
  * URL: /api/events/:groupId <!-- If we are creating an event for a specific groups details we need to be in the group router. "/groups/:groupId/events" We should not mismatch the router event with a group id.-->
  * Headers:
    * Content-Type: application/json
  * Body:
...
* Request
  * Method: POST
  * URL: /api/eventImages/:eventId <!-- We are creating an image tied to an event, it should be in the event router. "/api/events/:eventId/image"-->
  * Headers:
    * Content-Type: application/json
  * Body:
...
* Require Authentication: true
* Request
  * Method: PUT <!-- Should be a POST method, they need to create the record in the join table, this should default the status to pending-->
  * URL: /api/groups/:groupId/members
  * Headers:
    * Content-Type: application/json
  * Body: none
...
* Request
  * Method: GET
  * URL: /api/events/:eventId <!-- We need to specify we are looking for the attendees to the event. /api/events/:eventId/attendees -->
  * Body: none
...
* Request
  * Method: POST
  * URL: /api/events/:eventId <!-- We should be specifying that we are posting a new record in the join table by adding "attendance" to the url, "/api/events/:eventId/attendance"-->
  * Headers:
    * Content-Type: application/json
  * Body: none
...
* Request
  * Method: PUT
  * URL: /api/events/:eventId <!-- Specify we are editing attendance. "/api/events/:eventId/attendance"-->
  * Headers:
    * Content-Type: application/json
  * Body:
...
* Request
  * Method: DELETE
  * URL: /api/events/:eventId <!-- Need to specify we are deleting an attendance to the event, this would be for deleting an event. should be "/api/events/:eventId/attendance/:userId"-->
  * Headers:
    * Content-Type: application/json
  * Body: none
